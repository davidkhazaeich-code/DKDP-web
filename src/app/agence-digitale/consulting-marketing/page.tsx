  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest mb-2">
        ...
        <span style={{ color: '#A78BFA' }}>Objectif 6 mois DKDP</span>
      </div>
      {pillars.map((p) => (
        <div key={p.label} className="space-y-1.5">
          <div className="flex justify-between items-center">
            <span className="text-white text-xs font-semibold">{p.label}</span>
            <span className="text-[11px] font-bold" style={{ color: '#A78BFA' }}>+{p.after - p.before}pts</span>
          </div>
          ...
          <div className="flex justify-between text-[10px] text-text-muted">
            <span>{p.before}%</span>
            <span style={{ color: '#A78BFA' }}>{p.after}%</span>
          </div>
        </div>
      ))}
    </div>
