...
export function ThreatLevelChart() {
  const threats = [
    { name: 'Phishing', pct: 92, level: 'CRITICAL' },
    { name: 'Social engineering', pct: 88, level: 'CRITICAL' },
    { name: 'Weak passwords', pct: 82, level: 'HIGH' },
    { name: 'Ransomware', pct: 78, level: 'HIGH' },
    { name: 'Outdated software', pct: 55, level: 'MEDIUM' },
  ]
  return (
    <div className="space-y-4 w-full">
      ...
      {threats.map((t) => (
        <div key={t.name} className="space-y-1.5">
          ...
          <p className="text-text-muted text-[10px]">{t.pct}% of SMEs affected</p>
        </div>
      ))}
    </div>
  )
