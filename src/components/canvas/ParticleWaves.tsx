...
export function ParticleWaves({
  ...
  useEffect(() => {
    ...
    const pointT = new Float32Array(totalPoints)         // fade progress 0..1
    const pointPhase = new Float32Array(totalPoints)     // breath phase offset
    const pointExiting = new Uint8Array(totalPoints)     // fading out
    ...
    function animate() {
      ...
      for (let ix = 0; ix < DENSITY; ix++) {
        for (let iy = 0; iy < DENSITY; iy++) {
          ...
          posArr[i * 3 + 1] = y
          // Sync spike/pulse Y positions (only if visible, not parked at -99999)
